    ...
    >
      ...
      >
        <Box>
          ...
          >
            {respondent.email} · {respondent.fields.length} field{respondent.fields.length !== 1 ? "s" : ""}
          </Typography>
        </Box>
        <button
          type="button"
        ...
        {respondent.fields.map((f, i) => (
          <Box key={i}>
            ...
            >
              ...
              >
                {f.value || (f.jsonValue ? JSON.stringify(f.jsonValue, null, 2) : "—")}
              </Typography>
              {f.fileUrl && (
                <a
                  href={f.fileUrl}
  ...
  return (
    ...
    >
      ...
      >
        <Box>
          <Typography
            sx={{ fontSize: 15, fontWeight: 600, color: "#f4f4f5", fontFamily: "'Syne', sans-serif" }}
          >
            Propose Edit
          </Typography>
          <Typography
            sx={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontFamily: "'DM Mono', monospace", mt: 0.25 }}
          >
        ...
        {!pending && (
          <button
            type="button"
            onClick={onClose}
            style={{ background: "transparent", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.3)", padding: 4, lineHeight: 0, flexShrink: 0 }}
          >
            <X size={16} />
          </button>
        )}
      ...
      <Box sx={{ px: 3, py: 2.5, overflowY: "auto", flex: 1 }}>
        ...
        >
          <Typography
            sx={{ fontSize: 12, color: "rgba(168,85,247,0.8)", fontFamily: "'Syne', sans-serif", display: "flex", alignItems: "center", gap: 0.75 }}
          >
            <Pencil size={12} />
            All fields are pre-filled with current values. Only modified fields will appear in the proposal diff.
          </Typography>
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: "0 16px" }}>
          ...
          <DarkSelect label="Event Type" value={form.eventType} onChange={set("eventType")}>
            <option value="COMPETITION" style={{ background: "#0e0e0e" }}>Competition</option>
            <option value="WORKSHOP" style={{ background: "#0e0e0e" }}>Workshop</option>
            <option value="EVENT" style={{ background: "#0e0e0e" }}>Event</option>
          </DarkSelect>
        </Box>
        <DarkInput label="Short Description" value={form.shortDescription} onChange={set("shortDescription")} />
        <DarkInput label="Category" value={form.category} onChange={set("category")} placeholder="e.g. Robotics, Coding…" />
        <DarkTextarea label="Rules" value={form.rulesRichText} onChange={set("rulesRichText")} placeholder="Competition rules and guidelines…" rows={4} />

        {/* ── Schedule ── */}
        <SectionLabel>Schedule</SectionLabel>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: "0 16px" }}>
          <DarkDateTimeInput label="Start Time" value={form.startTime} onChange={set("startTime")} />
          <DarkDateTimeInput label="End Time" value={form.endTime} onChange={set("endTime")} />
        </Box>

        {/* ── Venue ── */}
        <SectionLabel>Venue</SectionLabel>
        <DarkInput label="Venue Name" value={form.venueName} onChange={set("venueName")} />
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: "0 16px" }}>
          <DarkInput label="Room" value={form.venueRoom} onChange={set("venueRoom")} placeholder="e.g. Lab 3" />
          <DarkInput label="Floor" value={form.venueFloor} onChange={set("venueFloor")} placeholder="e.g. Ground Floor" />
        </Box>

        {/* ── Team Settings ── */}
        <SectionLabel>Team Settings</SectionLabel>
        <DarkSelect label="Participation Type" value={form.type} onChange={set("type")}>
          <option value="SOLO" style={{ background: "#0e0e0e" }}>Solo (Individual)</option>
          <option value="TEAM" style={{ background: "#0e0e0e" }}>Team</option>
        </DarkSelect>
        {isTeam && (
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr 1fr" }, gap: "0 16px" }}>
            <DarkNumberInput label="Min Team Size" value={form.minTeamSize} onChange={set("minTeamSize")} min={1} />
            <DarkNumberInput label="Max Team Size" value={form.maxTeamSize} onChange={set("maxTeamSize")} min={1} />
            <DarkNumberInput label="Max Teams / College" value={form.maxTeamsPerCollege} onChange={set("maxTeamsPerCollege")} min={1} />
          </Box>
        )}
        {isTeam && (
          <InlineToggle label="Auto-approve teams" checked={form.autoApproveTeams} onChange={set("autoApproveTeams")} />
        )}

        {/* ── Registration ── */}
        <SectionLabel>Registration</SectionLabel>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: "0 16px" }}>
          <DarkDateTimeInput label="Registration Deadline" value={form.registrationDeadline} onChange={set("registrationDeadline")} />
          <DarkNumberInput label="Max Registrations" value={form.maxRegistrations} onChange={set("maxRegistrations")} min={1} placeholder="Unlimited" />
        </Box>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: "0 16px" }}>
          <DarkNumberInput label="Registration Fee (₹)" value={form.registrationFee} onChange={set("registrationFee")} min={0} />
        </Box>
        <Box
          sx={{
        ...
        >
          <InlineToggle label="Registrations Open" checked={form.registrationsOpen} onChange={set("registrationsOpen")} />
          <InlineToggle label="Requires Approval" checked={form.requiresApproval} onChange={set("requiresApproval")} />
          <InlineToggle label="Paid Event" checked={form.isPaid} onChange={set("isPaid")} />
          <InlineToggle label="Fee Per Person" checked={form.perPerson} onChange={set("perPerson")} />
          <Box sx={{ mb: 0 }}>
            <InlineToggle label="Attendance Required" checked={form.attendanceRequired} onChange={set("attendanceRequired")} />
          </Box>
        </Box>
  ...
  return (
    ...
    >
      ...
      {open && (
        <Box>
          {members.map((reg, i) => {
            const sc = REG_STATUS[reg.status] || { label: reg.status || "—", color: "rgba(255,255,255,0.4)" };
            return (
...
export default function ClubCompetitionDetailPage() {
  ...
  if (!competition) {
    return (
      <Box sx={{ p: { xs: 2, md: 4 } }}>
        <Typography
          sx={{ color: "rgba(255,255,255,0.35)", fontSize: 13, fontFamily: "'Syne', sans-serif" }}
        >
  ...
  const ec = EVENT_TYPE_CONFIG[competition.eventType] || EVENT_TYPE_CONFIG.COMPETITION;

  const tabList = [
    { key: "registrations", label: "Registrations", icon: Users, count: registrations.length },
    { key: "responses", label: "Form Responses", icon: FileText, count: respondentGroups.length },
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1100 }}>
          ...
          {registrationsLoading ? (
            ...
            <Box>
              ...
              >
                ...
                >
                  {registrationGroups.length} group{registrationGroups.length !== 1 ? "s" : ""} · {registrations.length} registrant{registrations.length !== 1 ? "s" : ""}
                </Typography>
              </Box>
      ...
      {tab === "proposal" && (
        ...
        >
          ...
          >
            As a Club Head you can propose edits to this competition. Proposals are reviewed by an SA before being applied.
          </Typography>
          <PurpleBtn onClick={() => setProposalOpen(true)}>
            <Pencil size={13} />
            Open Edit Proposal
